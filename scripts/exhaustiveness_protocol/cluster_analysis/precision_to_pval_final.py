	...
	for ic,cluster in enumerate(cluster_members):
		for member in cluster:
			model_index=models_subset[member]
                        print member,model_index,"is same"
...
def percent_ensemble_explained(ctable,num_models_per_run,total_num_models):
...
pklFile=sys.argv[3]
precision_convergence=float(sys.argv[4])
...
# This above will be the reduced contingency table, with cluster indices from cluster_members/cluster_centers retained
